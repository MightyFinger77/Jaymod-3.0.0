...
`jay_fixedAspect` (default `1`) is the widescreen HUD/UI switch. ETL often archives `cg_fixedAspect 0`, so that cvar is **not** used. Unset or `1` keeps 4:3 element sizes: HUD uses the extra width (left stuff stays left, right stuff stays right), menus / limbo / tab / load and exit screens stay a centered 640×480 panel, limbo sides use the same olive as the panel (not black), chat sits on the bottom edge of the screen, and in-game text (chat, kill feed, SPECTATOR, scoreboard names) uses TTF so it does not stretch. Centered HUD strings (first blood, center print, objective print, Connection Interrupted, crosshair names) use real TTF width instead of 8px cells. Tab/intermission win art sits on the centered 4:3 board. `jay_fixedAspect 0` is the old stretched 640×480 look.
